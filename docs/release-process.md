...
**判据**：一个搜到这条 issue、但不读代码的用户，能否看懂问题是否解决、从哪个版本生效、自己要做什么。回复写结论和用户可执行步骤，不写排障过程。

- 不暴露内部标识符、文件路径、函数名、提交哈希；用户需要的环境变量、配置项和命令可以写。
- 不写排障过程与自我检讨，也不附测试与验收清单；这些内容放在 PR、提交说明或维护记录里。
- **长度**：通常几句话，最多 20 行、最多 2 个小标题。
- 发出前用 `python dev/check_issue_reply.py draft.md --issue issue.md` 做机械检查，再按上面的判据人工通读。
