...
func findSSEGlobalAuthFile(t *testing.T) string {
	t.Helper()
	dirs := []string{"../data/auths", "../../data/auths", "../../../data/auths", "/root/workbuddy2api/data/auths"}
...
// loadSSEGlobalAcct 从 data/auths/ 读取真实 global 账号。每次调用重新解析磁盘（独立实例）。
