...
// 不调用 SaveAtomic（绝不改 data/auths/ 落盘凭证）；token 只以脱敏形式出现在日志。
package upstream
...
func findGlobalAuthFile(t *testing.T) string {
	t.Helper()
	dirs := []string{"data/auths", "../data/auths", "../../data/auths", "../../../data/auths", "/root/workbuddy2api/data/auths"}
...
// loadGlobalAcct 从 data/auths/ 读取真实 global 账号。每次调用重新解析磁盘文件
...
func TestGlobalE2ERefreshToken(t *testing.T) {
	...
	// 本轮在内存改写了 token：不落盘（禁止改 data/auths/），后续测试各自重新解析磁盘。
}
...
func TestGlobalE2EGatewayChat(t *testing.T) {
	...
	t.Logf("  注#10: 选号/出站域/日志见容器 docker logs（uid 见 data/auths/ 下 global 账号，host workbuddy.ai）")
}
