...
class UpstreamUpdateGuardTest(unittest.TestCase):
    ...
    def test_integrated_windows_update_is_manual(self) -> None:
        from server import config
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'):
            ok, msg = self._start('upstream', False)
        self.assertFalse(ok)
        self.assertIn('Windows', msg)

    def test_integrated_windows_update_is_manual_for_both(self) -> None:
        from server import config
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'):
    ...
    def test_integrated_windows_update_is_manual_even_with_docker(self) -> None:
        """Native Windows still uses the explicit root restart workflow."""
        from server import config
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'):
    ...
    def test_integrated_windows_manager_update_is_manual(self) -> None:
        """The single native service is restarted through the root script."""
        from server import config
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'):
    ...
    def test_status_reports_capability(self) -> None:
        ...
        from server import config
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'), \
                mock.patch.object(updater, 'can_control_docker', return_value=False):
            st = updater.read_status()
        self.assertTrue(st['can_update_upstream'])
    ...
    def test_capability_not_derived_from_container(self) -> None:
        ...
        from server import config
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'), \
                mock.patch.object(updater, 'in_container', return_value=False), \
                mock.patch.object(updater, 'can_control_docker', return_value=False):
...
class ComposeCommandTest(unittest.TestCase):
    ...
    def test_dockerfile_builds_embedded_gateway(self) -> None:
        """单镜像必须构建并复制 Go 网关。"""
        df = (_ROOT / 'Dockerfile').read_text(encoding='utf-8')
        self.assertIn('FROM golang:1.22', df)
        self.assertIn('COPY --from=go-builder', df)
        self.assertIn('/usr/local/bin/wb2api', df)
        self.assertNotIn('docker.sock', df)

    def test_gateway_build_is_arch_neutral(self) -> None:
        """Go builder follows the target Docker platform without host CLI downloads."""
        df = (_ROOT / 'Dockerfile').read_text(encoding='utf-8')
        self.assertIn('FROM golang:1.22-bookworm AS go-builder', df)
        self.assertNotIn('download.docker.com', df)

    def test_compose_is_not_required_inside_image(self) -> None:
        """The unified image does not need Docker CLI or Compose at runtime.
        ...
        """
        df = (_ROOT / 'Dockerfile').read_text(encoding='utf-8')
        self.assertNotIn('COMPOSE_VERSION', df)
        self.assertNotIn('cli-plugins/docker-compose', df)
    ...
    def test_unified_compose_has_one_service(self) -> None:
        import yaml
        dc = yaml.safe_load((_ROOT / 'docker-compose.yml').read_text(encoding='utf-8'))
        self.assertEqual(list(dc['services']), ['wb2api'])
...
class DockerAssetsTest(unittest.TestCase):
    ...
    def test_dockerfile_present_and_non_root(self) -> None:
        ...
        self.assertIn('WB_RUN_MODE=integrated', df,
                      '镜像里没设统一运行形态')
        self.assertIn('WB2API_MODE=integrated', df)
    ...
    def test_compose_passes_npm_registry_arg(self) -> None:
        ...
        import yaml
        dc = yaml.safe_load((_ROOT / 'docker-compose.yml').read_text(encoding='utf-8'))
        args = (dc['services']['wb2api'].get('build') or {}).get('args') or {}
    ...
    def test_compose_persists_data(self) -> None:
        import yaml
        dc = yaml.safe_load((_ROOT / 'docker-compose.yml').read_text(encoding='utf-8'))
        vols = dc['services']['wb2api'].get('volumes') or []
    ...
    def test_docker_socket_is_not_required(self) -> None:
        """The embedded gateway removes the need for host Docker control.
        ...
        dc = yaml.safe_load((_ROOT / 'docker-compose.yml').read_text(encoding='utf-8'))
        vols = [str(v) for v in (dc['services']['wb2api'].get('volumes') or [])]
        self.assertFalse(any('docker.sock' in v for v in vols))
...
class ContainerReloadHintTest(unittest.TestCase):
    ...
    def _save(docker_ok: bool) -> dict:
        ...
        with mock.patch.object(config, 'WB2API_MODE', 'integrated'), \
                mock.patch.object(st.updater, 'can_control_docker', return_value=docker_ok), \
                mock.patch.object(st.wb2api, 'save_upstream_config',
    ...
    def test_integrated_mode_does_not_need_docker(self) -> None:
        res = self._save(docker_ok=False)
        self.assertTrue(res['reload_scheduled'])
...
class MultiArchImageTest(unittest.TestCase):
    ...
    def test_dockerfile_does_not_hardcode_x86_64(self) -> None:
        df = (_ROOT / 'Dockerfile').read_text(encoding='utf-8')
        self.assertNotIn('download.docker.com', df)
        self.assertNotIn('x86_64', df)

    def test_dockerfile_maps_arch_names(self) -> None:
        """The Go builder is selected by Docker and needs no host CLI mapping."""
        df = (_ROOT / 'Dockerfile').read_text(encoding='utf-8')
        self.assertIn('FROM golang:1.22-bookworm AS go-builder', df)
        self.assertNotIn('TARGETARCH', df)
