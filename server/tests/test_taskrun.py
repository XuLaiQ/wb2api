...
class ScriptExtractionTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._orig_upstream = config.UPSTREAM_DIR
        ...
        # 把上游目录指到一个空目录：本组测试要测的是「宿主机没有脚本 →
        # 从容器 docker cp 提取」这条路径，前提是 `_host_script()` 找不到东西。
        #
        # 为什么以前不写也能过：UPSTREAM_DIR 的原默认值是 /opt/workbuddy2api，
        # 在开发机上并不存在，于是「恰好」满足前提。合并成一个仓库后，上游就在
        # <仓库>/upstream，默认值推导会命中真实存在的 scripts/task_runner.py，
        # 提取路径被短路（这是**正确的**行为，见 test_host_mount_wins_over_extraction），
        # 本组测试便以「冷却没生效，试了 0 次」这种看不出原因的方式失败。
        # 显式指定之后，本组测试与部署形态无关。
        config.UPSTREAM_DIR = Path(self._tmp.name) / 'no-upstream'
        taskrun._last_extract_failure.update({'at': 0.0, 'reason': ''})

    def tearDown(self) -> None:
        config.DATA_DIR = self._orig_data
        config.AUTH_DIR = self._orig_auth
        config.UPSTREAM_DIR = self._orig_upstream
        taskrun._last_extract_failure.update({'at': 0.0, 'reason': ''})
        self._tmp.cleanup()
...
class EventLoopResponsivenessTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        config.AUTH_DIR = Path(self._tmp.name) / 'auths'
        config.AUTH_DIR.mkdir(parents=True, exist_ok=True)
        # 同 ScriptExtractionTest.setUp：让「宿主机没有脚本」这个前提显式成立。
        # 本组测试模拟的是**脚本缺失时** docker 调用会不会冻住事件循环 ——
        # 若 _host_script() 命中了真实的 <仓库>/upstream/scripts/task_runner.py，
        # 脚本根本不缺失，整组测试的前提就没了。
        config.UPSTREAM_DIR = Path(self._tmp.name) / 'no-upstream'
        taskrun._last_extract_failure.update({'at': 0.0, 'reason': ''})
