"""运行期配置：Go 网关、FastAPI 和前端共用同一项目根目录。"""
...
# Embedded workbuddy2api gateway (Go)
WB2API_BASE = _env('WB2API_BASE', 'http://127.0.0.1:7863').rstrip('/')
WB2API_KEY = _env('WB2API_KEY', '')
WB2API_CONTAINER = _env('WB2API_CONTAINER', 'wb2api')
WB2API_MODE = _env('WB2API_MODE', 'integrated').lower()  # integrated | docker | native
WB2API_BINARY = Path(_env(
    'WB2API_BINARY', str(ROOT / ('wb2api.exe' if os.name == 'nt' else 'wb2api')),
))
...
# 本管理端数据
DATA_DIR = Path(_env('WB_DATA_DIR', str(ROOT / 'data' / 'manager')))
