...
背景
----
workbuddy2api 只在进程启动时读取 config.json 与扫描 data/auths/ 目录
（见 gateway/cmd/server/main.go：auth.LoadDir + p.SyncToDir + 各 SetXxx 注入），
之后不再重读，也不处理 SIGHUP。因此任何影响这两者的改动都必须重启容器。

**一处例外**（上游 2026-09-18 起）：auths 目录加了热加载——每 5 秒轮询目录指纹，
凭证文件增删改会自动重新加载，那时不必重启（见其 gateway/internal/pool/watch.go）。
