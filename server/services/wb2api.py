...
async def restart_container() -> tuple[bool, str]:
    """重启 Go 网关；合并项目默认由 embedded 生命周期管理器负责。"""
    if config.WB2API_MODE == 'integrated':
        from . import embedded

        return await embedded.restart()
