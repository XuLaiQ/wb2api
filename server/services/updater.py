...
def read_status() -> dict:
    ...
    status: dict = {
        ...
        'can_update_upstream': (
            config.WB2API_MODE == 'integrated' or can_control_docker()
        ),
        # 合并布局标记：上游是本仓库的子目录，版本与更新语义都不同
        # （见 _is_merged_layout 与 deploy/update.py 的 _update_merged_repo）。
        # 前端可据此调整文案；不做判断时忽略它也不会出错。
        'merged_layout': _is_merged_layout(),
    }
...
def start_update(target: str) -> tuple[bool, str]:
    ...
    env.update({
        ...
        'WB_SERVICE_NAME': os.environ.get('WB_SERVICE_NAME', 'wb2api'),
        'WB2API_MODE': config.WB2API_MODE,
        'WB2API_BINARY': str(config.WB2API_BINARY),
...
def _is_merged_layout() -> bool:
    """上游是否就在本仓库里（合并布局）。

    判据是**目录包含关系**而不是「文件存在」：上游目录位于仓库根之下，
    说明它随本仓库一起被 clone、一起被更新，两者版本天然相同。

    为什么需要这个判断：原管理端假设上游是一个**独立的 git 仓库**
    （`Sliverkiss/workbuddy2api`），于是「上游版本」取自上游仓库的 HEAD、
    「是否有更新」要拿上游的 commit 去和上游远端比。合并后这套假设不成立 ——
    upstream/ 只是本仓库的一个子目录，git 会向上找到仓库根的 .git，
    返回的是**本仓库**的 HEAD。不修正的话，版本页会拿本仓库的 commit
    去和上游原仓库比对，compare API 必然 404（本地 commit 不在那个仓库里），
    结果静默显示「无更新」——不报错，但永远不准。
    """
    try:
        return config.UPSTREAM_DIR.resolve().is_relative_to(config.PROJECT_ROOT.resolve())
    except Exception:  # noqa: BLE001
        return False


def _git_remote_slug() -> str:
    """从本仓库的 git remote 推导出 GitHub 的 owner/name；推不出来返回空串。

    为什么不直接硬编码合并后的仓库名：这是个**模板仓库**，用户会 fork 到自己
    账号下再部署。写死一个别人的地址，版本检测就永远查的是别人的仓库。
    从 remote 推导能自动跟随 fork，且不需要额外配置。

    推不出来（没配 remote / 不是 GitHub / 不是 git 仓库）时返回空串，
    由调用方回退到环境变量与历史默认值。
    """
    try:
        proc = subprocess.run(
            ['git', 'remote', 'get-url', 'origin'],
            cwd=str(config.PROJECT_ROOT), capture_output=True, text=True, timeout=10,
        )
...
# 合并布局下两个「仓库」其实是同一个：版本检测统一指向本仓库。
#
# 回退值是**空串**而不是原仓库名：查一个不属于自己的仓库，得到的「无更新」是假的
# —— 本地 commit 根本不在那个仓库里，compare API 必然 404，界面会静默显示
# 「已是最新」。空串会让版本页明确显示「未配置仓库地址」，比一个看起来正常但
# 永远错误的结论诚实得多。配好 git remote（或设 WB_MANAGER_REPO）即自动恢复。
_IS_MERGED = _is_merged_layout()
_DETECTED_REPO = _git_remote_slug() if _IS_MERGED else ''
UPSTREAM_API_REPO = (
    os.environ.get('WB_UPSTREAM_API_REPO') or _DETECTED_REPO
    or ('' if _IS_MERGED else 'Sliverkiss/workbuddy2api')
)
# 管理端仓库（owner/name），用于查询最新 Release
MANAGER_REPO = (
    os.environ.get('WB_MANAGER_REPO') or _DETECTED_REPO
    or ('' if _IS_MERGED else 'ithtelab/workbuddy-manager')
)
...
def _upstream_api_slug() -> str:
    ...
    合并布局下「上游仓库」就是本仓库，slug 已在模块加载时解析好
    （见上面的 `_IS_MERGED` / `UPSTREAM_API_REPO`），这里**不再读
    `WB_UPSTREAM_REPO`** —— 那个变量是给「上游是独立仓库」的部署用的，
    在合并布局下它指向的是原上游仓库，拿本仓库的 commit 去和它比对必然 404。
    """
...
def _local_upstream_head(full: bool = False) -> str:
    ...
    合并布局下的语义变化：`upstream/` 不再是独立 git 仓库，`git rev-parse HEAD`
    会向上找到仓库根的 .git，返回的正是**本仓库**的 HEAD —— 这正是我们想要的，
    因为合并后上游与本仓库同版本。所以这里不需要按布局分支，两种布局下
    这个调用都给出正确结果。
    """
    try:
...
def _fetch_remote_versions() -> dict:
    ...
    if not MANAGER_REPO and not UPSTREAM_API_REPO:
        # 合并布局下既没有 git remote 也没设环境变量 —— 无从查起。
        # 明确说出来，而不是让两次请求各自失败、在界面上留一串看不懂的 HTTP 错误。
        hint = ('未配置仓库地址：本仓库还没有 git remote。请执行 '
                '`git remote add origin <你的仓库地址>`，或设置 WB_MANAGER_REPO')
        result['manager']['error'] = hint
        result['upstream']['error'] = hint
        return result
