...
UPSTREAM_DIR = Path(os.environ.get('WB_UPSTREAM_DIR') or INSTALL_DIR)


def _unified_layout() -> bool:
    """The Go gateway and manager are one repository and one deployment unit."""
    mode = (os.environ.get('WB2API_MODE') or '').strip().lower()
    return mode == 'integrated' or UPSTREAM_DIR.resolve() == INSTALL_DIR.resolve()

def _upstream_is_standalone_repo() -> bool:
    """上游是否为**独立 git 仓库** —— 区分「合并布局」与「拆分布局」的唯一可靠信号。

    刻意不看路径形态（比如「upstream 是否在 INSTALL_DIR 的上一级」）：容器部署时
    upstream/ 被挂到 /opt/workbuddy2api，与宿主上的真实路径并不一致，按路径猜必然
    出错。而「有没有自己的 .git」在两种部署形态下含义一致 —— 合并仓库里
    upstream/ 只是一个子目录，永远不会自带 .git。

    **刻意写成函数而不是模块级常量**：UPSTREAM_DIR 可以在导入之后被改
    （环境变量、测试的 mock），常量会在导入那一刻就固定下来，之后怎么改都不生效。
    test_docker_deploy.py 正是「先导入模块、再创建 UPSTREAM_DIR/.git」的顺序 ——
    常量形态下它会误判成合并布局，从而跳过整段独立仓库的更新逻辑。
    """
    return not _unified_layout() and (UPSTREAM_DIR / '.git').is_dir()


def _repo_dir() -> Path:
    """合并布局下本仓库的根目录（upstream/ 的上一级）。

    拆分布局下这个值没有意义，但也不会被读到 —— update_upstream 会先分支。
    """
    return INSTALL_DIR if _unified_layout() else UPSTREAM_DIR.parent
...
SERVICE_NAME = os.environ.get('WB_SERVICE_NAME') or 'wb2api'
...
def _default_branch(repo: Path, rep: Reporter) -> str:
    """仓库的默认分支名。

    为什么要问远端而不是写死：合并后的仓库由用户自己创建，默认分支可能是 main
    也可能是 master（GitHub 老账号建的仓库仍是 master）。写死任一个，在另一种
    上 git pull 都会失败。
    顺序：origin/HEAD 指向的分支 → 本地当前分支 → 'main'（新建仓库的常见默认）。
    """
    rc, out = run(['git', 'symbolic-ref', '--short', 'refs/remotes/origin/HEAD'],
                  cwd=repo, rep=rep, check=False)
...
def _rebuild_all(rep: Reporter, repo: Path) -> None:
    """按仓库根的 compose 重建统一应用容器。"""
    compose_file = repo / 'docker-compose.yml'
    if not compose_file.is_file():
        rep.log(f'未找到 {compose_file}，跳过容器重建。'
                '代码已更新，请手动重启服务使其生效', 'warn')
        return
    compose_cmd = _compose_cmd()
    if compose_cmd is None:
        rep.log('当前环境没有可用的 docker compose 命令，请在宿主机执行：', 'warn')
        rep.log(f'  cd {repo} && docker compose up -d --build', 'warn')
        return
    rep.log('重建并启动统一 wb2api 容器（首次可能需数分钟）…')
    run(compose_cmd + ['-f', str(compose_file), 'up', '-d', '--build'],
        cwd=repo, rep=rep, check=False)


def _update_merged_repo(rep: Reporter) -> None:
    """合并布局下的「更新上游」：更新整个仓库。

    合并成一个仓库后，上游与管理端同版本 —— 单独更新上游在技术上不可能
    （upstream/ 没有自己的 git 历史），语义上也没有意义。所以这里把操作提升为
    「更新整个仓库」，并同时重建两个容器。

    两类运行环境的差别决定了能不能全自动完成：

      · 宿主机部署（systemd / 直接跑）：仓库根的 .git 可见，可以真正
        git pull + docker compose up -d --build，全程自动；
      · 容器部署：compose 只把 upstream/ 子目录挂进容器，仓库根的 .git 不在
        容器里，无法执行 git。此时**明确告知在宿主机怎么更新**，而不是静默跳过
        —— 静默跳过会让人以为「点了没反应 = 已经是最新版」。
    """
    repo = _repo_dir()
    if not (repo / '.git').exists():
        rep.log('上游位于本仓库内（upstream/），随整个仓库一起更新。')
        rep.log('但当前环境看不到仓库根目录 —— 容器部署通常只挂载了 upstream/ '
    ...
    if dirty:
        ...
        backup = DATA_DIR / 'repo-local-changes.patch'
        rc, diff = run(['git', 'diff'], cwd=repo, rep=rep, check=False)
        if diff.strip():
            backup.write_text(diff, encoding='utf-8')
            rep.log(f'  已备份到 {backup}')
        run(['git', 'checkout', '--', '.'], cwd=repo, rep=rep, check=False)

    # 2) 拉取
    rc, out = run(['git', 'rev-parse', 'HEAD'], cwd=repo, rep=rep, check=False)
    before = out.strip()[:8] if rc == 0 else ''
    branch = _default_branch(repo, rep)

    run(['git', 'fetch', '--depth', '1', 'origin', branch], cwd=repo, rep=rep, check=False)
    rc, out = run(['git', 'pull', '--ff-only', 'origin', branch],
                  cwd=repo, rep=rep, check=False)
    if rc != 0:
        rep.log('fast-forward 失败，尝试硬重置到远端（本地改动已备份）', 'warn')
        run(['git', 'reset', '--hard', f'origin/{branch}'], cwd=repo, rep=rep, check=False)

    rc, out = run(['git', 'rev-parse', 'HEAD'], cwd=repo, rep=rep, check=False)
    after = out.strip()[:8] if rc == 0 else ''
    if before and after and before == after:
        rep.log(f'代码已是最新（{after}）')
    else:
        rep.log(f'代码更新：{before or "?"} → {after or "?"}')

    # 3) 重建唯一的统一容器
    _rebuild_all(rep, repo)


def update_upstream(rep: Reporter) -> None:
    rep.step('更新上游 workbuddy2api')

    # 合并布局：上游没有独立的 git 历史，「只更新上游」不再是独立操作。
    if not _upstream_is_standalone_repo():
        _update_merged_repo(rep)
        return
...
def update_manager(rep: Reporter) -> None:
    ...
    # 合并布局下这条路径的覆盖范围要讲清楚，否则容易误解成「更新完了」。
    if not _upstream_is_standalone_repo():
        rep.log('提示：本仓库把上游与管理端合在一起，这一步只替换管理端部分'
                '（server/ 与 web/out/），不会动 upstream/。'
                '要连上游一起更新，请用「更新上游」或直接在仓库根 git pull。')
